...
register_test(test_something_else, VERBOSE_FAIL_LOG) {
  ...
  simple_assert_float(6.6, 6.6, 0.0001F);   // NOLINT
  simple_assert_double(7.3, 7.4, 0.0001F);  // NOLINT
...
register_test(test_something_true, VERBOSE_FAIL_LOG) {
  simple_assert_double(0.0, 0.0, 0.001F);  // NOLINT
  simple_assert_string("Hello world", "Hello world");
  simple_assert_nonnull((void *)1);
}

register_param_test(test_params, ARGS_LIST(int sum1, char sum2),  // NOLINT
                    VERBOSE_FAIL_LOG, PARAM_LIST(0, 'a'), PARAM_LIST(1, 'b'),
                    PARAM_LIST(2, 'c')) {
